...
async function searchCharacterScope(query) {
    ...
    const matches = await searchOneCharacterAllChats(query, character, $results, {
        onProgress: (i, total, fileName) => {
            $results.html(`<div class="cs-loading">${i}/${total} 채팅 파일 검색 중... (${escapeHtml(fileName)})</div>`);
        },
    });
...
function renderResults($container, matches, query, opts = {}) {
    ...
    for (const match of matches) {
        ...
        const langBadge = match.matchedTranslated ? '<span class="cs-badge">🌐 번역본</span>' : '';
        const charBadge = opts.showCharBadge && match.charName
            ? `<span class="cs-badge cs-badge-char">${escapeHtml(match.charName)}</span>`
            : '';
        const $row = $(`
            <div class="cs-row">
                <div class="cs-meta">
                    ${charBadge}
                    <span class="cs-file">${escapeHtml(String(match.fileName))}</span>
                    <span class="cs-badge">${badge}</span>
                    ${langBadge}
                    <span class="cs-name">${escapeHtml(match.name || '')}</span>
                    <span class="cs-star ${bookmarked ? 'cs-star-on' : ''}" title="북마크">${bookmarked ? '★' : '☆'}</span>
                </div>
        ...
        $row.on('click', (e) => {
            if ($(e.target).is('.cs-star')) return; // 별 클릭은 아래에서 따로 처리
            expanded = !expanded;
            if (expanded) {
        ...
        $star.on('click', (e) => {
            ...
            if ($star.hasClass('cs-star-on')) {
                removeBookmark(id);
                $star.removeClass('cs-star-on').text('☆');
            } else {
                addBookmark({ id, ...match, query });
                $star.addClass('cs-star-on').text('★');
            }
        });
...
function renderBookmarksView() {
    ...
    for (const b of bookmarks) {
        const badge = b.isSystem ? '👻 숨김' : (b.isUser ? '🧑 유저' : '🤖 AI');
        const charBadge = b.charName ? `<span class="cs-badge cs-badge-char">${escapeHtml(b.charName)}</span>` : '';
        const $row = $(`
            <div class="cs-row">
                <div class="cs-meta">
                    ${charBadge}
                    <span class="cs-file">${escapeHtml(String(b.fileName))}</span>
                    <span class="cs-badge">${badge}</span>
                    <span class="cs-name">${escapeHtml(b.name || '')}</span>
                    <span class="cs-star cs-star-on" title="북마크 해제">★</span>
                </div>
...
function buildUI() {
            ...
            <div class="cs-panel">
                <div class="cs-header">
                    ...
                    <div class="cs-header-actions">
                        <span id="cs-bookmarks-btn" class="cs-icon-btn" title="북마크 보기">🔖</span>
                        <span id="cs-close" class="cs-close">✕</span>
                    </div>
                </div>
                <div class="cs-scope-bar">
                    <select id="cs-scope" class="cs-select">
                        <option value="current">현재 채팅</option>
                        <option value="character" selected>캐릭터 채팅 전체</option>
                        <option value="all">전체 채팅 (모든 캐릭터)</option>
                    </select>
                    <select id="cs-char-select" class="cs-select"></select>
                </div>
    ...
    $('#cs-scope').on('change', updateScopeUI);
    ...
    $('#cs-bookmarks-btn').on('click', () => {
        ...
        if (showingBookmarks) {
            ...
            $('#cs-results').show();
            $('.cs-scope-bar, .cs-searchbar').show();
            $('#cs-bookmarks-btn').removeClass('cs-icon-btn-active');
        } else {
            renderBookmarksView();
            $('#cs-results').hide();
            $('.cs-scope-bar, .cs-searchbar').hide();
...
jQuery(async () => {
    buildUI();
    console.log('[chat-searching] 로드됨 (v2: 검색범위/캐릭터선택/북마크)');
});
