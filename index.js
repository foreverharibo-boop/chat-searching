...
async function jumpToChat(fileName) {
    const bareFileName = fileName.replace(/\.jsonl$/, '');
    const context = SillyTavern.getContext();

    // 1순위: getContext()에 이미 openCharacterChat이 있으면 그걸 바로 사용 (가장 안전함)
    if (typeof context.openCharacterChat === 'function') {
        try {
            await context.openCharacterChat(bareFileName);
            closeModal();
            return;
        } catch (err) {
            console.error('[chat-searching] context.openCharacterChat 실패', err);
        }
    }

    // 2순위: 코어 script.js에서 직접 동적 import (절대경로 사용 -> 설치 위치/폴더 깊이 상관없음)
    try {
        const core = await import(/* webpackIgnore: true */ '/script.js');
        if (typeof core.openCharacterChat === 'function') {
            await core.openCharacterChat(bareFileName);
            closeModal();
            return;
        }
